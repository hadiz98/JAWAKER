/**
 * Trix — 2-player trick-taking game. Full deck, 26 cards each.
 * Must follow lead suit if possible; highest card in lead suit wins the trick.
 * Game ends after 13 tricks; winner is the one who took more tricks.
 */
...
class TrixGameRules implements IGameRules {
  ...
  isValidMove(state: GameState, move: Move, playerId: string): boolean {
    ...
    if (currentTrick.length === 0) return true; // leading any card is valid
  ...
  getWinner(state: GameState): string | null {
    ...
    if (t0 + t1 < totalTricks) return null;
